...
// each last element to the vertical order traversal will be added to answer array 
...
class Solution
{
    ...
    // to understand the process loook for solution of vertical width question
        if(root == null) return;
    ...
    
    {
        ...
        for(int i=0;i<len;i++){
            int subArrayLen=result.get(i).size();
            ans.add(result.get(i).get(subArrayLen-1));// adding last element of each inner array to the answer array
        }
        return ans;
